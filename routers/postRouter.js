...
const joiBoard = Joi.object({
    postingTitle: Joi.array().items(Joi.string()).required(),
    postingComment: Joi.array().items(Joi.string()).required(),
    postingDate: Joi.array().items(Joi.string()).required(),
    postingUpdate: Joi.array().items(Joi.string()).required(),
    postingImgUrl: Joi.array().items(Joi.string()),
    postingDel: Joi.array().items(Joi.number()).required(),
});
...
Router.post('/posting', authMiddleWare, async(req, res) => {
    ...
    const { postingTitle, postingDate, postingComment, postingImgUrl, postingDel } = joiBoard.validateAsync(req.body);
    try {
...
Router.patch("/postModify", async(req, res, next) => {
    try {
        const { postID, postingTitle, postingComment, postingUpdate } = joiBoard.validateAsync(req.body);
